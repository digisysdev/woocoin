...
uint256 hashGenesisBlock("0x1a1181a7188d1a527bc2a91d0d89dabcfb6a50fab387fcc4fe6f4bb5e17a8e02");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Woocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Woocoin Signed Message:\n";
...
{
    // Woocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Woocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Woocoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Woocoin: 30 seconds
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Woocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1a1181a7188d1a527bc2a91d0d89dabcfb6a50fab387fcc4fe6f4bb5e17a8e02");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Woocoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x7be5aea080b651baf3962390fba7502c3cb7077dc97cd3af1a93bcd610aae8e5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Woocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WoocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WoocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WoocoinMiner(CWallet *pwallet)
{
    printf("WoocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("woocoin-miner");
    ...
    try { loop {
        ...
        printf("Running WoocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WoocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WoocoinMiner, pwallet));
}
